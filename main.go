...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	meanSpeedSec := w.meanSpeed() * KmHInMsec
	heightInM := w.Height / CmInM
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(meanSpeedSec, 2)/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
